# -*- coding: utf-8 -*-
"""calendario_tareas_lao.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1xdOKe7nqDYRQVucD-PG7GGr7DZsC6dHk
"""
...
# Intentamos establecer el locale, pero si falla (común en entornos web sin los paquetes de idioma),
# usaremos la traducción manual de seguridad más abajo.
...
def format_date_in_spanish(date_obj):
    """
    Formatea la fecha a español de manera segura.
    Si locale.setlocale funciona, la fecha ya estará en español.
    Si no, realiza una traducción manual para asegurar el idioma.
    """
    # Intentamos obtener la fecha usando el locale (puede ser español o inglés)
    # Formato: "DíaSemana, Día de Mes de Año"
    date_str = date_obj.strftime("%A, %d de %B de %Y")

    # Capitalizar la primera letra (Día de la semana)
    date_str = date_str.capitalize()

    # Traducción manual de seguridad si el locale falló (la fecha está en inglés)
    for en, es in SPANISH_DAYS.items():
        # Reemplazar tanto el nombre completo capitalizado como en minúsculas
        date_str = date_str.replace(en, es)
        date_str = date_str.replace(en.lower(), es.lower())
...
def load_tasks():
    ...
    if os.path.exists(TASKS_FILE):
        try:
            ...
            st.warning("El archivo de tareas está corrupto. Se ha iniciado con una lista vacía.")
            return []
    return []
...
def save_tasks():
    ...
    # Pre-procesar tareas: convertir objetos date a cadenas ISO para serialización
    tasks_to_save = []
...
def update_task_date(task_id, new_date):
    ...
    for i, t in enumerate(st.session_state.tasks):
        if t["id"] == task_id:
            ...
            save_tasks() # Guardar después de actualizar
            return True
    return False
...
def move_task_to_calendar_callback(task_id):
    ...
    if new_date:
        if update_task_date(task_id, new_date):
            # La modificación del estado de sesión y el guardado dispararán el rerun de Streamlit.
            pass
...
st.header("🗓️ Calendario de tareas")
